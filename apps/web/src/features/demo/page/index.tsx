...
function Row({ children, header }: { children: React.ReactNode, header: string }) {
  return (
    ...
    <div className="w-screen h-screen p-0 m-0 flex flex-col items-center">
      ...
      <div className='w-full flex flex-col p-30 overflow-y-scroll'>
        <Row header="Walkers' favorite">
          {commerce1.map((item) => <Card {...item} />)}
        </Row>
        <Row header='Top seller'>
          {commerce2.map((item) => <Card {...item} />)}
        </Row>
        <Row header='Trending'>
          {commerce3.map((item) => <Card {...item} />)}
        </Row>
        <Row header='Books'>
          {book.map((item) => <Card {...item} />)}
        </Row>
        <Row header='Vehicles'>
          {vehicle.map((item) => <Card {...item} />)}
        </Row>
      </div>
    </div>
  );
